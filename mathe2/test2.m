...
Tf_cos = 0;
for k = 0:N
    Tf_cos = Tf_cos + (-1)^k * (pi/2)^k * x.^(2*k) / factorial(2*k);
end
...
Tf_sin = 0;
for k = 0:N
    Tf_sin = Tf_sin + (-1)^k * (pi/2)^k * x.^(2*k+2) / factorial(2*k+2);
end

% Berechnung des Integrals
sin_result = integral(@(x) [Tf_cos; Tf_sin], 0, I(2), 'ArrayValued', true);
